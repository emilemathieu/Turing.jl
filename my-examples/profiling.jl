...
@model infiniteMixtureStick(y) = begin
  N = length(y)
  P = rand(DP(alpha, [Normal(meanMean, 1.0/sqrt(meanPrecision)),Gamma(precisionShape, 1.0/precisionInvScale)], false))
...
@model infiniteMixtureLazy(y) = begin
  N = length(y)
  P = rand(DP(alpha, [Normal(meanMean, 1.0/sqrt(meanPrecision)),Gamma(precisionShape, 1.0/precisionInvScale)], true))
  # P = rand(mLogBetaPK(1.0, 2.0, [Normal(meanMean, 1.0/sqrt(meanPrecision)),Gamma(precisionShape, 1.0/precisionInvScale)]))
